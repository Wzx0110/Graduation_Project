...
GENERATION_NEGATIVE_PROMPT = "blurry, low quality, deformed, distorted, unrealistic, bad anatomy, disfigured, text, words, letters, signature, watermark"  # 負面提示詞，避免生成特定內容
...
if __name__ == "__main__":
    ...
    try:
        # 以二進制讀取模式打開圖片檔案
        with open(INPUT_IMG1_PATH, 'rb') as f1, open(INPUT_IMG2_PATH, 'rb') as f2:
            img1_bytes = f1.read()
            img2_bytes = f2.read()
        print(f"成功載入圖片: {INPUT_IMG1_PATH}, {INPUT_IMG2_PATH}")
    except Exception as e:
        print(f"載入輸入圖片時發生錯誤: {e}")
        exit()
