...
def generate_subtitle(descriptions: List, model_name: str = 'phi4') -> Optional[List[Dict[str, str]]]:
    ...
    prompt = f"""You are given a list of scene transition descriptions. Use these to write a short, coherent story in Traditional Chinese, between **200 and 300 Traditional Chinese characters** in total.
    ...
    - Each line must contain **no more than 10 Traditional Chinese characters**
    - Avoid punctuation at the end of each line
    - Do not break words or phrases unnaturally

    Output JSON Format:
    Please respond with ONLY a valid JSON array of objects in the format specified above. Do NOT include any explanations, headers, code blocks, or markdown syntax.
